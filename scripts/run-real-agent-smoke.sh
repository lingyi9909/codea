...
# attempts them. The Unit Test workflow removes the fixture mvnw stub so
# run_project_test must invoke a real Maven install and execute the generated
# JUnit test. This is the real-runtime evidence behind Task 14/15's agentGate,
# not a string-match contract test.
#
...
# diff) and lay down the Maven fixture. Remove the deterministic mvnw stub so
# run_project_test falls back to the real `mvn` executable and really compiles
# and executes the JUnit generated by unit-test-generator.
...
echo "[PASS] real OpenCode agent smoke: fail-closed tools + generated JUnit executed by real Maven"
