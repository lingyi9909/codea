...
type agentPickerModel struct { Visible bool; Items []runtime.Agent; Cursor int }
func (p *agentPickerModel) Open(items []runtime.Agent,current string){p.Visible=len(items)>0;p.Items=append([]runtime.Agent(nil),items...);p.Cursor=0;current=strings.TrimSpace(current);for i,item:=range p.Items{if item.Name==current{p.Cursor=i;break}}}
...
func (m *Model) handleProfessionalWorkspaceMessage(msg tea.Msg)(bool,tea.Cmd){
	if handled,cmd:=m.handleModelCheckMessage(msg);handled{return true,cmd}
	switch msg:=msg.(type){
	...
	case eventStreamClosedMsg:
		if m.modelCheck.Active{m.failModelCheck("Model qualification stopped because the Runtime event stream closed.")};return false,nil
	case listAgentsResultMsg:
		if msg.err!=nil{m.appendInfo("Failed to load agents: "+msg.err.Error());return true,nil};if len(msg.agents)==0{m.agentPicker.Close();m.appendInfo("No runtime agents are available.");return true,nil};m.agentPicker.Open(msg.agents,m.currentAgent);m.markDirty();return true,nil
	case loadHistoryResultMsg:
		if m.pendingResumeID!=""&&msg.sessionID==m.pendingResumeID{m.agentPicker.Close()};return false,nil
	case tea.KeyMsg:
...
func (m *Model) handleAgentPickerKey(msg tea.KeyMsg)tea.Cmd{switch{case key.Matches(msg,m.keys.Up):m.agentPicker.MoveUp();case key.Matches(msg,m.keys.Down):m.agentPicker.MoveDown();case key.Matches(msg,m.keys.Esc):m.agentPicker.Close();case key.Matches(msg,m.keys.Submit):selected,ok:=m.agentPicker.Selected();m.agentPicker.Close();if !ok||strings.TrimSpace(selected.Name)==""{m.appendInfo("Agent selection expired.");return nil};m.currentAgent=selected.Name;m.appendInfo("Agent selected for this session: "+selected.Name)};m.markDirty();return nil}
func (m *Model) agentPickerView()string{if !m.agentPicker.Visible{return ""};var b strings.Builder;b.WriteString("Agents (runtime)\n");for i,agent:=range m.agentPicker.Items{prefix:="  ";if i==m.agentPicker.Cursor{prefix="> "};mode:="";if strings.TrimSpace(agent.Mode)!=""{mode=" ("+agent.Mode+")"};fmt.Fprintf(&b,"%s%s%s\n",prefix,agent.Name,mode)};b.WriteString("↑/↓ select · Enter apply · Esc close");return b.String()}
