...
type TaskExecutionState struct {
	...
	VerificationContinuationLimit int
...
func (m *Model) resetTaskExecution(turnID string) {
	...
	limit := m.activeModelStrategy.VerifyContinuationLimit
	if limit <= 0 {
		limit = 2
	}
	m.taskExecution = TaskExecutionState{
		RootTurnID:                   root,
		VerificationContinuationLimit: limit,
...
func (m *Model) recordMessageRoot(messageID, rootTurnID string) {
	...
	if m.taskExecution.messageRoots == nil { m.taskExecution.messageRoots = make(map[string]string) }
...
func (m *Model) observeTaskExecutionEvent(ev runtime.Event) {
	...
	if m.taskExecution.RootTurnID != m.activeTurnID { m.resetTaskExecution(m.activeTurnID) }
	if ev.SessionID != "" && m.sessionID != "" && runtime.SessionID(ev.SessionID) != m.sessionID { return }
	if ev.MessageID != "" {
	...
	if ev.Type == eventTypeToolCalled && mutationExecutionTool(tool) { markMutation(&m.taskExecution) }
	if tool == "verify_project" { observeVerificationToolEvent(&m.taskExecution, ev); return }
	if ev.Type != eventTypeToolSuccess { return }
	if tool != "task_plan" && tool != "task_step" && tool != "task_status" { return }
	applyTaskPlanMetadata(&m.taskExecution, ev.Tool.Metadata)
}

func mutationExecutionTool(tool string) bool {
	switch strings.TrimSpace(tool) {
	case "write", "edit", "bash", "write_test_file", "write_document", "run_project_test": return true
...
func observeVerificationToolEvent(state *TaskExecutionState, ev runtime.Event) {
	if state == nil || ev.Tool == nil { return }
	callID := strings.TrimSpace(ev.Tool.CallID)
	if state.verificationCalls == nil { state.verificationCalls = make(map[string]struct{}) }
	if state.verificationCallEpoch == nil { state.verificationCallEpoch = make(map[string]int) }
	if ev.Type == eventTypeToolCalled {
		if callID == "" { return }
		if _, seen := state.verificationCalls[callID]; seen { return }
	...
	if !started || callEpoch != state.mutationEpoch { return }
	...
	if ev.Type != eventTypeToolSuccess || ev.Tool.Metadata["codeaVerification"] != "true" { return }
	...
	if !validVerificationResult(result) || !validVerificationProfile(profile) { return }
...
func validVerificationResult(result string) bool {
	switch result { case "pass", "fail", "timeout", "not_configured", "error": return true; default: return false }
}
func validVerificationProfile(profile string) bool {
	switch profile { case "maven", "gradle", "go", "unknown": return true; default: return false }
}

func applyTaskPlanMetadata(state *TaskExecutionState, metadata map[string]string) {
	if state == nil || metadata["codeaTaskPlan"] != "true" { return }
	total, errTotal := strconv.Atoi(metadata["codeaPlanTotal"])
	completed, errCompleted := strconv.Atoi(metadata["codeaPlanCompleted"])
	if errTotal != nil || errCompleted != nil || total < 3 || total > 7 || completed < 0 || completed > total { return }
	active := strings.TrimSpace(metadata["codeaPlanActive"])
	if len(active) > 100 { return }
	state.PlanSeen = true
...
func (m *Model) renderTaskProgress() string {
	...
	if !state.PlanSeen || state.TotalSteps < 3 || state.TotalSteps > 7 { return "" }
	if state.RootTurnID != m.activeTurnID { return "" }
	base := fmt.Sprintf("Plan · %d/%d", state.CompletedSteps, state.TotalSteps)
	if m.viewMode == ViewFocus { return base }
	base += " steps"
	if state.ActiveStep != "" { base += " · " + state.ActiveStep }
	return base
}
