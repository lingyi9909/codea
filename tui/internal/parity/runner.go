...
const (
	...
	terminalDrainDelay = 500 * time.Millisecond
)
...
func (r *Runner) Run(ctx context.Context, s Scenario) ScenarioResult {
	...
	allPassed := make([]bool, 0, repeats)
	for i := 0; i < repeats; i++ {
		allPassed = append(allPassed, r.executeOnce(ctx, s, &sr))
	}
	sr.Runs = repeats
	for _, passed := range allPassed {
		if !passed {
			return sr
		}
	}
...
func (r *Runner) runPrompt(ctx context.Context, s Scenario, sr *ScenarioResult) {
	...
	if s.Assertions.RequireAgent != "" {
		...
		if !bOK || !cOK {
			missing := []string{}
			if !bOK { missing = append(missing, "baseline") }
			if !cOK { missing = append(missing, "candidate") }
	...
	if a.RequireRaw {
		for _, e := range events {
			if e.Type == "raw" && len(e.Raw) > 0 {
				var v any
				if json.Unmarshal(e.Raw, &v) == nil { goto rawOK }
			}
		}
...
func computeFingerprint(events []runtime.Event) eventFingerprint {
	...
	for _, e := range events {
		fp.counts[e.Type]++
		switch e.Type {
		case "answer.delta": fp.answerChars += len(e.Content)
		case "reasoning.delta": fp.reasoningChars += len(e.Content)
		case "tool.called": fp.toolCalls++
		case "approval.requested": fp.approvals++
		case "raw": fp.rawEvents++
		case "step.finished": fp.hasStepFinish = true
		}
	}
	return fp
}
...
func compareFingerprints(bEvents, cEvents []runtime.Event, a Assertion) []string {
	bFP, cFP := computeFingerprint(bEvents), computeFingerprint(cEvents)
	var issues []string
	...
	for typ := range bFP.counts {
		if cFP.counts[typ] == 0 { issues = append(issues, fmt.Sprintf("candidate missing event type %q present in baseline (%d events)", typ, bFP.counts[typ])) }
	}
	return issues
}

func hasEventType(events []runtime.Event, typ runtime.EventType) bool {
	for _, e := range events { if e.Type == typ { return true } }
	return false
}
...
func (r *Runner) collectEvents(ctx context.Context, rt runtime.AgentRuntime, req *runtime.PromptRequest, approvalDecision *runtime.ApprovalDecision, timeout time.Duration) ([]runtime.Event, error) {
	...
	for {
		...
		case ev, ok := <-ch:
			...
			// session.idle is the real runtime's canonical terminal event and is
			// often mapped through Raw fallback. Append it before returning so the
			// RawEventHandling release gate can prove zero silent loss.
			if ev.RawType == "session.idle" { return events, nil }
			if ev.Type == "step.finished" || ev.Type == "step.failed" {
				terminalDrain = time.After(terminalDrainDelay)
			}
		case <-terminalDrain:
			return events, nil
		case <-deadline:
...
func ConcatContent(events []runtime.Event, typ runtime.EventType) string {
	...
	for _, e := range events { if e.Type == typ { b.WriteString(e.Content) } }
	return b.String()
}
