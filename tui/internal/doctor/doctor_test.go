...
func TestDoctorSkipsBehaviorWhenRuntimeStartupFailed(t *testing.T){home,cfg:=doctorInstall(t);svc,_:=NewDefaultService(Config{HomeDir:home,ConfigDir:cfg,RuntimeStartError:errors.New("runtime boom"),RuntimeURL:"http://127.0.0.1:4141"});report:=svc.Run(context.Background());if resultByName(t,report,"Runtime 健康").Status!=Fail{t.Fatal("startup failure must fail health")};if resultByName(t,report,"SSE").Status!=Skip||resultByName(t,report,"模型推理").Status!=Skip{t.Fatal("behavior checks must skip without runtime")}}
...
func TestInferenceFailureIsReported(t *testing.T){home,cfg:=doctorInstall(t);rt:=&fakeRuntime{health:runtimedomain.HealthInfo{Healthy:true,Version:"1.18.11"},promptErr:errors.New("model unavailable")};svc,_:=NewDefaultService(Config{HomeDir:home,ConfigDir:cfg,Runtime:rt,RuntimeURL:"http://127.0.0.1:4141",ExpectedOpenCodeVersion:"1.18.11",BehaviorTimeout:time.Second});report:=svc.Run(context.Background());if resultByName(t,report,"模型推理").Status!=Fail{t.Fatalf("inference should fail: %s",FormatText(report))}}
