...
func TestBuiltinsAreRegisteredAndFilterable(t *testing.T) {
	...
	all := reg.Commands()
	if len(all) != 19 {
		t.Fatalf("builtins = %d, want 19 through Task 32", len(all))
	}
	...
	for _, name := range []string{"help", "clear", "status", "sessions", "skills", "agents", "cancel", "doctor", "model", "model-check", "compact", "view", "review", "test", "api-doc", "debug", "checkpoint", "checkpoints", "restore"} {
		if !seen[name] {
...
func TestDefinitionOwnsCapabilityAndAvailabilityMetadata(t *testing.T) {
	reg := NewRegistry()
	want := Definition{Name: "cap-aware", Source: SourceBuiltin, Action: ActionStatus, RequiredCapability: "compaction", Agent: "general", Availability: AvailabilityUnavailable}
	if err := reg.Register(want); err != nil { t.Fatal(err) }
	got := reg.Commands()[0]
...
func TestParsePreservesCommandArguments(t *testing.T) {
	reg := NewRegistry()
	if err := reg.Register(Definition{Name: "review", Source: SourceBuiltin, Action: ActionReview}); err != nil { t.Fatal(err) }
	inv, err := reg.Parse("/review OrderService  --deep ")
	if err != nil { t.Fatal(err) }
	if inv.Definition.Name != "review" { t.Fatalf("name = %q, want review", inv.Definition.Name) }
	if inv.Arguments != "OrderService  --deep " { t.Fatalf("arguments = %q", inv.Arguments) }
}
...
func TestUnknownCommandIsDeterministic(t *testing.T) {
	...
	var cmdErr *Error
	if !errors.As(err, &cmdErr) { t.Fatalf("error = %T %v, want *Error", err, err) }
	if cmdErr.Code != CodeNotFound { t.Fatalf("code = %q, want %q", cmdErr.Code, CodeNotFound) }
}

func TestCollisionFailsClosedWithoutOverride(t *testing.T) {
	reg := NewRegistry()
	if err := reg.Register(Definition{Name: "doctor", Aliases: []string{"diag"}, Source: SourceBuiltin, Action: ActionDoctor}); err != nil { t.Fatal(err) }
	err := reg.Register(Definition{Name: "project-doctor", Aliases: []string{"doctor"}, Source: SourceProject, Action: ActionPrompt})
	var cmdErr *Error
	if !errors.As(err, &cmdErr) || cmdErr.Code != CodeConflict { t.Fatalf("collision error = %T %v, want %s", err, err, CodeConflict) }
	inv, err := reg.Parse("/doctor")
	if err != nil { t.Fatal(err) }
	if inv.Definition.Source != SourceBuiltin { t.Fatalf("doctor source = %q, want builtin", inv.Definition.Source) }
}
...
func TestFutureControlledBuiltinsAreReservedBeforeOwningTaskRegistersThem(t *testing.T) {
	for _, def := range []Definition{
		...
		{Name: "project-model-check", Aliases: []string{"model-check"}, Source: SourceProject, Action: ActionPrompt},
	} {
		reg := NewRegistry()
		err := reg.Register(def)
		var cmdErr *Error
		if !errors.As(err, &cmdErr) || cmdErr.Code != CodeConflict { t.Fatalf("Register(%#v) error = %T %v, want %s", def, err, err, CodeConflict) }
	}
	reg := NewRegistry()
	if err := reg.Register(Definition{Name: "review", Source: SourceBuiltin, Action: ActionReview}); err != nil { t.Fatalf("controlled built-in owner should be allowed to register /review: %v", err) }
	if err := reg.Register(Definition{Name: "view", Source: SourceBuiltin, Action: ActionView}); err != nil { t.Fatalf("controlled built-in owner should be allowed to register /view: %v", err) }
	if err := reg.Register(Definition{Name: "model-check", Source: SourceBuiltin, Action: ActionModelCheck}); err != nil { t.Fatalf("controlled built-in owner should be allowed to register /model-check: %v", err) }
}
...
func TestWorkspaceLoaderLoadsEnterpriseThenProjectAndRendersArguments(t *testing.T) {
	...
	if err != nil { t.Fatal(err) }
	if out.Kind != OutcomePrompt || out.Agent != "code-reviewer" { t.Fatalf("outcome = %#v", out) }
	if out.Prompt != "Review order:\nOrderService  --changed-only\n" { t.Fatalf("prompt = %q", out.Prompt) }
	inv, err := reg.Parse("/explain Foo.java")
	if err != nil { t.Fatal(err) }
	if inv.Definition.Source != SourceProject { t.Fatalf("source = %q, want project", inv.Definition.Source) }
}
...
func TestWorkspaceLoaderRejectsProjectOverrideOfBuiltin(t *testing.T) {
	...
	var cmdErr *Error
	if !errors.As(err, &cmdErr) || cmdErr.Code != CodeConflict { t.Fatalf("error = %T %v, want %s", err, err, CodeConflict) }
}
...
func TestWorkspaceLoaderRejectsProjectClaimOnFutureControlledBuiltin(t *testing.T) {
	...
	_, err := LoadWorkspaceRegistry("", project)
	var cmdErr *Error
	if !errors.As(err, &cmdErr) || cmdErr.Code != CodeConflict || cmdErr.Command != "review" { t.Fatalf("error = %T %v, want %s for /review", err, err, CodeConflict) }
}
