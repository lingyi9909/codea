...
Checkpoint: `03d37af5d6b9cae384d4c0639fb9afb860bd6888`

在 Task 10 的 Skill Manager 之上增加双轨 Skill 策略（strict/compatible），保证 Enterprise Agent 只加载 Approved+Enabled 的 Codea Skill（**真实隔离**，作用于实际 Runtime 加载，非 UI 假过滤），同时 General Agent 不丢失 OpenCode 原生 Project/User Skill 能力。
...
- **真实隔离**：strict 下 (1) `SyncEnabled` 只物化 Approved+Enabled Codea Skill 到受控 `OPENCODE_CONFIG_DIR/skills/`，(2) supervisor `buildEnv` 关闭 Project/User 扫描并重定向 XDG 隔离原生 User Skill 目录。
...
- `FilterForMode(skills, p)`：compatible 原样返回；strict 只保留 `StrictAllowed` 的 Codea Skill。
...
`Config.CodeaSkillsOnly bool`。strict 下 `buildEnv` 追加 `OPENCODE_DISABLE_EXTERNAL_SKILLS=1` + `OPENCODE_DISABLE_PROJECT_CONFIG=1`，并重定向 `XDG_CONFIG_HOME`/`XDG_DATA_HOME`/`XDG_CACHE_HOME`/`XDG_STATE_HOME` 到 `config.ConfigDir/xdg/…`（隔离原生 `~/.config/opencode/skills`）。HOME 不重定向（`~/.claude` 已被恒开的 `OPENCODE_DISABLE_CLAUDE_CODE=1` 关闭）。
...
|---------|------------------|------|
| Smoke A（compatible） | `['codea-skill', 'customize-opencode', 'project-skill', 'user-skill']` | Codea + Project + User 全部可加载，Native 不退化 |
| Smoke B（strict） | `['codea-approved', 'customize-opencode']` | 仅 approved Codea 可加载；project/user/unapproved 全部隔离 |
...
- [x] `SkillPolicy` + `FilterForMode` strict 只保留 approved Codea；compatible 保留全部
- [x] strict 物化只落盘 approved+enabled Codea（`SyncEnabled` + `Sync` 跳过 disabled）
- [x] supervisor strict 关闭 Project/User 扫描 + 重定向 XDG 隔离原生 User Skill（真实隔离，Smoke B 证明）
- [x] compatible 不追加 disable 标志、不重定向 XDG（Smoke A 证明 Native 不退化）
...
| `tui/internal/supervisor/supervisor_test.go` | Modify（`TestBuildEnvIsolation` 断言 disable 标志 + XDG 重定向） |
